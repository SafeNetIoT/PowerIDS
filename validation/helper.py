...
def verify_mask_groups_compact(df_pat, full_features_csv, top_n=25):
   ...
   # 6. Plotting - Custom Color & Size
    ...
    cbar.ax.tick_params(labelsize=26) # Big numbers
    ...
    ax_top.set_xticklabels(group_labels, fontsize=28, fontweight='normal') 
...
def plot_masking_performance(df_pat, perf_csv):
    ...
    for i, (cid, group) in enumerate(groups):
        ...
        mid = (start + end) / 2
        ax.text(mid, 103, f"M{cid}", ha="center", fontsize=28, weight="normal")

    # Lines
    ax.vlines(x, y_active, y_idle, color="#505050", linewidth=2.5, alpha=0.6, zorder=1)

    # Points (Dual Coding: Circle + Triangle)
    ax.scatter(x, y_idle, s=160, marker='o', color="#34669a", edgecolors="#34669a", 
               linewidth=1.5, label="Idle Baseline", zorder=2)

    ax.scatter(x, y_active, s=180, marker='^', color="#e0795f", edgecolors="#e0795f", 
    ...
    ax.set_ylabel("Detection F1 Score (%)", fontsize=28, labelpad=15)
    ax.set_xticks(x)
